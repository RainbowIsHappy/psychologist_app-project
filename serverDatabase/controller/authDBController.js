...
const handleLogin = async (req, res) => {
  ...
  if (match) {
    ...
    foundUser = await User.findOneAndUpdate({ email}, {refreshToken}, {
...
const handleLogout = async (req, res) => {
  ...
  try {
    ...
    if (!foundUser) 
      throw "Not found this refresh token" + refreshToken;
  } catch (error) {
    res.clearCookie("jwt", { httpOnly: true, sameSite: "None", secure: true, path:"/", maxAge:0 });
    return res.sendStatus(204);
  };
  // Delete refreshToken in db
  const result = await User.findOneAndUpdate({ email: foundUser.email}, {refreshToken:""}, {
